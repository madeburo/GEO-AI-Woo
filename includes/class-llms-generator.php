...
class Geo_Ai_Woo_LLMS_Generator {
    ...
    public function generate( $is_full = false, $lang_code = null ) {
        ...
        // Header
        $site_name = self::decode_text( get_bloginfo( 'name' ) );
        $output[]  = '# ' . $site_name;
        $output[]  = '';
        ...
        if ( $description ) {
            $output[] = '> ' . self::decode_text( $description );
            $output[] = '';
        }
        ...
        $section_labels = array(
            'page'    => __( 'Pages', 'geo-ai-for-woocommerce' ),
            'post'    => __( 'Blog Posts', 'geo-ai-for-woocommerce' ),
            'product' => __( 'Products', 'geo-ai-for-woocommerce' ),
        );
        ...
        // Footer
        $output[] = '---';
        $output[] = 'Generated by GEO AI Search Optimization v' . GEO_AI_WOO_VERSION;
    ...
    private function get_content( $post_type, $is_full = false ) {
        ...
        if ( $query->have_posts() ) {
            while ( $query->have_posts() ) {
                ...
                $post_id = get_the_ID();
                $title   = self::decode_text( get_the_title() );
                ...
                if ( ! empty( $ai_description ) ) {
                    $description = self::decode_text( $ai_description );
                } elseif ( 'product' === $post_type && class_exists( 'WooCommerce' ) ) {
                    $description = self::decode_text( apply_filters( 'geo_ai_woo_product_description', '', $post_id ) );
                } else {
                    $description = self::decode_text( wp_trim_words( Geo_Ai_Woo_Content_Sanitizer::sanitize( get_the_excerpt() ), 20, '...' ) );
                }
                ...
                if ( ! empty( $keywords ) ) {
                    $line .= ' [' . self::decode_text( $keywords ) . ']';
                }
                ...
                if ( $is_full ) {
                    ...
                    $content  = self::decode_text( wp_trim_words( $content, 200, '...' ) );
                    $output[] = '  ' . $content;
                    $output[] = '';
                }
            }
    ...
    /**
     * Decode text for plain-text output: strip HTML tags and decode entities to UTF-8.
     * Also repairs mojibake from double UTF-8 encoding (common with latin1 DB connections).
     *
     * @param string $text Text to decode.
     * @return string Clean UTF-8 text.
     */
    private static function decode_text( $text ) {
        $text = wp_strip_all_tags( $text );
        $text = html_entity_decode( $text, ENT_QUOTES | ENT_HTML5, 'UTF-8' );

        // Fix mojibake: detect double-encoded UTF-8 (latin1 → UTF-8 → UTF-8).
        // Common symptom: Cyrillic "Привет" stored as "ÐŸÑ€Ð¸Ð²ÐµÑ‚".
        if ( function_exists( 'mb_detect_encoding' ) && preg_match( '/[\xC0-\xDF][\x80-\xBF]/', $text ) ) {
            $attempt = @mb_convert_encoding( $text, 'UTF-8', 'Windows-1252' );
            if ( $attempt && mb_check_encoding( $attempt, 'UTF-8' ) && $attempt !== $text ) {
                // Verify the conversion actually produced valid multibyte characters
                // by checking if the result has fewer bytes (double-encoded → single-encoded).
                if ( strlen( $attempt ) < strlen( $text ) ) {
                    $text = $attempt;
                }
            }
        }
    ...
    private function get_taxonomy_sections( $post_types, $is_full = false ) {
        ...
        $taxonomies = array(
            'category' => __( 'Categories', 'geo-ai-for-woocommerce' ),
            'post_tag' => __( 'Tags', 'geo-ai-for-woocommerce' ),
        );

        // Add WooCommerce taxonomies if active
        if ( in_array( 'product', $post_types, true ) && class_exists( 'WooCommerce' ) ) {
            $taxonomies['product_cat'] = __( 'Product Categories', 'geo-ai-for-woocommerce' );
            $taxonomies['product_tag'] = __( 'Product Tags', 'geo-ai-for-woocommerce' );
        }
        ...
        foreach ( $taxonomies as $taxonomy => $label ) {
            ...
            foreach ( $terms as $term ) {
                ...
                $term_name = self::decode_text( $term->name );
                $term_desc = $term->description ? self::decode_text( $term->description ) : '';
                $line      = "- [{$term_name}]({$term_url})";

                if ( $term_desc ) {
                    $line .= ': ' . self::decode_text( wp_trim_words( $term_desc, 15, '...' ) );
                }
    ...
    public function add_robots_txt_rules( $output, $public ) {
        ...
        $bot_rules = isset( $settings['bot_rules'] ) ? $settings['bot_rules'] : array();
        $additions = "\n# GEO AI Search Optimization — AI Crawler Rules\n";
    ...
    /**
     * Write a single file using direct file_put_contents with UTF-8 safety.
     *
     * @param string $file_path    Full path to the file.
     * @param string $file_content File content to write.
     */
    private function write_file( $file_path, $file_content ) {
        // Ensure content is valid UTF-8 before writing.
        // Some server configurations or DB connections may produce corrupted multibyte strings.
        ...
        // Write directly — WP_Filesystem FTP transport can corrupt multibyte characters,
        // so we use file_put_contents for static .txt files in ABSPATH.
